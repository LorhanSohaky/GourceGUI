...
void init_entry(_string *destination);
...
void init_entry(_string *destination){
    destination->value = (char *) malloc(ENTRY_LENGTH* sizeof(char));
    if(destination->value!=NULL){
        for(int i=0;i<ENTRY_LENGTH-1;i++){
            (destination->value)[i]=' ';
        }
        (destination->value)[ENTRY_LENGTH-1]='\0';
        destination->size=ENTRY_LENGTH;
    }else{
        destination->size=0;
